...
- `args` 中的 DataModel 参数使用完整表达式；列表循环项字段用相对表达式，例如 `"{{ ${entityId} }}"`；静态固定目标直接写 JSON 值。
...
- 列表循环项内可用 `$item/$index`，也可通过 `children.itemVar/indexVar` 自定义变量名；本 Skill 仍兼容 `${field}` 解析到当前数组项字段，`${/rootField}` 解析到根 DataModel。
- `id`、`component`、对象 key、EventHandler `call`、`updateDataModel.path`、CardSpec `writeResultTo`、列表循环 `children.path` 和整个 `styles` 对象不支持表达式。
...
- 列表循环仅用于 `Row`、`Column`、`List` 的 `children`，循环对象必须包含 `componentId` 和 `path`，可选 `itemVar`、`indexVar`。
- 列表循环 `children.path` 指向数组；循环项内可用 `$item/$index` 或自定义变量名，绝对 `${/field}` 解析到根。
