...
- 触发素材检查看内容职责，不靠业务名枚举：对象需要被快速识别、状态需要图形化、动作需要方向/播放/拨打等视觉指示、主媒体或地点需要可视锚点，或设计正准备用文字字形/自绘图形/背景图替代素材时，都先查表。
- 用户未提供素材不等于素材不可用；本表声明的本地素材就是可用素材。
- 如果存在明确匹配的素材，且卡片仍需要该语义图标、图片、媒体或视觉锚点，必须使用匹配素材的 `src`。
- 只在 Design Compact 骨架中承担明确职责时使用素材：对象识别、状态、动作、主媒体、hero 或精确背景。不得用素材数量填充空白；2x2 通常一个主素材并可按需增加 1–2 个更小的状态/对象/动作素材，2x4 可按分区、序列或列表使用多个小型辅助素材。
- 匹配成功后，不要用 `Text` 字形、emoji、自绘形状、相似资源路径、未声明 SVG 或未声明图片替代该语义素材。
- SVG 默认可使用 `fillColor`，颜色必须根据直接背景与当前色彩角色选择；多色、品牌、渐变、透明或描述要求保留原色的素材不染色。PNG 永不使用 `fillColor`。
- Image 必须显式写 `width`、`height`、`objectFit`。只有描述明确为背景/壁纸的素材才能作为背景使用；背景承担 canvas 职责，不占主素材名额，但使用后应降低前景素材尺寸、数量或对比度。
- 只有没有语义匹配素材、加入素材会破坏固定骨架预算，或用户明确要求不用图片/图标素材时，才省略 `Image`。
...
- 图标按 Design Compact 角色取值：header `20vp`，常规语义 `16–24vp`，胶囊按钮 `20vp`，独立 icon 动作中心 `16–20vp`，hero 在 2x2 为 `40–56vp`、2x4 为 `48–72vp`。
- 标题与右上 icon 间隔 `4vp`；胶囊按钮 icon 与文字间隔 `8vp`；先扣除 icon 和 gap，再估算文本宽度。
- hero 图标只能在所选骨架存在明确 hero 角色时使用；不能把普通图标自由放大成装饰主视觉。
