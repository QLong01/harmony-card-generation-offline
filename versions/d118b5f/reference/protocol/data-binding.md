...
- `updateDataModel.path`、CardSpec `writeResultTo`、列表循环 `children.path` 是协议结构 JSON Pointer，不属于值绑定，仍按协议保留。
- 可见表达式引用必须能从 `updateDataModel.value`、CardSpec `writeResultTo + outputSchema` 或列表循环当前项推导。
- `writeResultTo` 对应动态子树允许使用示例初始值；该子树必须是能力 `outputSchema` 的合法投影，字段名、嵌套层级、JSON 类型、枚举、数值范围和数组 `items` 结构均不得偏离 schema。
- 列表循环只用于 `Row`、`Column`、`List` 的 `children`，循环对象只有 `componentId` 和 `path`；循环项内字段读取也用表达式。
...
- `id`、`component`、对象 key、EventHandler `call`、`updateDataModel.path`、CardSpec `writeResultTo`、列表循环 `children.path` 和整个 `styles` 对象不能写表达式。
...
- 列表循环 `children.path`，例如 `"/items"`。

这些结构路径只负责定位数据写入或循环源；组件真正展示字段时仍用完整表达式。

## 列表循环

列表循环是协议特性，仅在确实需要重复数据时使用。Form 支持默认 `$item/$index`，也支持 `itemVar/indexVar` 自定义变量名；本 Skill 兼容 `${field}` 相对字段写法。
...
- 列表循环组件及其子树内，`$item.field` / `$index` 或自定义 `$product.name` / `$i` 读取当前项与索引；`${field}` 也解析到当前数组项字段，`${/rootField}` 解析到根 DataModel。
- 循环项内拼接仍写成完整表达式；简单直取可以使用 PathBinding，但不要把 `{ "path": "field" }` 误写成非 JSON Pointer。
- 同一层列表循环如果 `itemVar` 与 `indexVar` 同名，按最终协议回退为默认 `$item/$index`。
...
- 每个可见表达式引用的数据都能从 DataModel、列表循环当前项或 `writeResultTo + outputSchema` 推导。
- 每个宿主动作或 event capability 参数来自 DataModel、列表循环当前项、事件上下文或合法静态目标。
- 每个列表循环来源路径都指向数组。
