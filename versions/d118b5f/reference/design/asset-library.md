...
- 只在 Pixso 明确槽位使用素材：可选右上标题 icon、内容图片区、按钮 icon 或 ring 中心 icon。不得用素材数量填充空白，也不得扩展到未经定义的 2×4 视觉。
...
- 图标尺寸只按 Pixso 角色取值：标题 icon `20vp`，胶囊按钮 icon `20vp`，纯 icon 按钮中心 icon `16vp`，环形图中心 icon `24vp`。
- 标题与右上 icon 间隔 `4vp`；胶囊按钮 icon 与文字间隔 `8vp`；先扣除 icon 和 gap，再估算文本宽度。
- Pixso 0804 未定义自由“主视觉图标”尺寸；不要使用旧版 44-72vp 范围自行放大素材。
