          ...
          "pages": [
            {
              ...
              "description": "点击一键加入下一个日程对应的Welink会议。uri取自数据能力GetCalendarEvents返回结果中event的oneClickServiceLink字段。注意：列表循环中events/i的i需替换为当前事件的实际索引，如events/0、events/1等。"
            }
          ]
        }
      ]
...
- 事件参数可以来自安全静态值、DataModel 绝对路径，或列表循环项的相对路径。来自 data capability 输出的字段，必须能从 `writeResultTo + outputSchema` 推导。
...
- 列表循环项内使用当前项字段时优先写相对路径，例如 `{"path": "entityId"}`；列表循环外使用绝对路径，例如 `{"path": "/data/calendar/items/0/entityId"}`。
