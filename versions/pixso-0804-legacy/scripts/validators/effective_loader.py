...
3. ``capabilities_dir/{data,event,asset}_capabilities.json`` from an external
   capability registry.
...
def attach_effective_capabilities(
    ...
    normalized["event"] = _resolve_effective_event_capabilities(
        normalized["event"],
        capabilities_dir,
    )
...
def _resolve_effective_event_capabilities(
    items: list[Any],
    capabilities_dir: Path | None,
) -> list[Any]:
    if capabilities_dir is None:
        return items
    event_map = _load_capability_map(capabilities_dir / "event_capabilities.json")
    resolved: list[Any] = []
    for item in items:
        if isinstance(item, str) and item in event_map:
            resolved.append(event_map[item])
        elif isinstance(item, dict):
            item_id = item.get("id") or item.get("capabilityId")
            if isinstance(item_id, str) and item_id in event_map:
                resolved.append(event_map[item_id])
            else:
