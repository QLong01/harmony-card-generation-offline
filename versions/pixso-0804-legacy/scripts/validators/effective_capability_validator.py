...
from .capability_schema import schema_errors
...
class EffectiveCapabilityValidator(BaseValidator):
        ...
        for component in context.components:
            ...
            for index, handler in enumerate(handlers):
                ...
                reporter.add(
                    ...
                    fix_hint="Use only event actions that passed effective capability filtering.",
                )
    ...
    def _event_actions(self, items: list[Any]) -> list[dict[str, Any]]:
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            action = item.get("actionTemplate")
            ...
            result.append(
                {
                    "id": item.get("id") or item.get("capabilityId"),
                    "call": call,
                    "args": args,
                    "parametersSchema": item.get("parametersSchema"),
                    "dynamicArguments": item.get("dynamicArguments", []),
                }
            )
        return result
    ...
    def _event_allowed(
        ...
        for item in allowed_actions:
            if item.get("call") != call:
                continue
            parameter_schema = item.get("parametersSchema")
            if isinstance(parameter_schema, dict):
                dynamic_paths = {
                    dynamic.get("path")
                    for dynamic in item.get("dynamicArguments", [])
                    if isinstance(dynamic, dict) and isinstance(dynamic.get("path"), str)
                }
                if not schema_errors(args, parameter_schema, dynamic_paths=dynamic_paths):
                    return True
                continue
            template = {"call": item.get("call"), "args": item.get("args", {})}
            if self._stable_json(template) == expected:
                return True
        return False
